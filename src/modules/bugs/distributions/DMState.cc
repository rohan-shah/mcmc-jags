...
/*
double DMState::logLikelihood(long x, vector<double const *> const &par) const
{
  int initial = 0;
  double time = 0;
  int nstate = 0;
  double const *intensity = 0;
  getPar(&initial, &time, &nstate, &intensity, par);
...
long DMState::randomSample(vector<double const *> const &par) const
{
  int initial = 0;
  double time = 0;
  int nstate = 0;
