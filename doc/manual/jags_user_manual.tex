...
\ldots R-1, R$.\footnote{If $L > U$ then the operator $:$ returns a
...
Node arrays can be traversed with for loops. In the linear regression
example above, a for loop is used to define nodes \verb+Y[i]+ and
\verb+mu[i]+ for values of $i$ ranging from $1$ to $M$.
