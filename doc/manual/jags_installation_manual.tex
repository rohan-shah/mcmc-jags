...
App Store, which is a fairly large download but contains a full set of
compilers and other development tools.  A command line utility
\code{xcode-select} is also available for installation of compilers, but
this does not install some of the libraries necessary to build \JAGS.
The instructions that follow assume you are running OS 10.9 or later,
and have the free Xcode application from the Mac App Store.

\subsection{Required tools}

If you wish to build from a released source package i.e.
\file{JAGS-\release.tar.gz}, you will need Xcode.  You will also need to
agree to the Xcode agreement before using the compilers (if you have not
...
the command \code{sudo xcodebuild -license} (or simply \code{sudo clang -v}) 
...
`X.X.X' is the version number.  You now need to open the Terminal and
change the working directory to the \JAGS\ source code. In the Terminal
window after the \$ prompt type 'cd' followed by a space.  In the Finder
drag JAGS-X.X.X folder into the Terminal window and hit return.  If this
worked for you, typing 'ls' followed by a return will list the contents
of the JAGS folder.
